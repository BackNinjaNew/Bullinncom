...
@section('content')
<div class="row px-16">
    <h1 class="text-5xl font-bold text-center pt-10 pb-10">Mis Compras</h1>
    @if (count($sales) > 0)
    <form method="POST" action="{{ route('catalog.search_buys') }}">
        @csrf
        ...
        <table class="w-full text-md text-left rtl:text-right text-gray-600">
            ...
            <tbody>
                ...
                <tr class="text-center bg-blue-300 border-2 hover:bg-blue-400">
                    ...
                    <td class="px-6 py-1">
                        <a href="{{ route('catalog.detail_buys', $item->invoice) }}" type="button" class="rounded-md bg-green-600 w-9 text-lg text-white text-center font-bold p-2 my-3 hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-green-300"><i class="fa fa-eye"></i></a>
                    </td>
                </tr>
                @endforeach
            </tbody>
